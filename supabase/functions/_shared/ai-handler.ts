...
`
    ...
    if (agent.business_hours?.enabled && agent.business_hours.schedule) {
      ...
      const weekday = timeParts.find((p: any) => p.type === 'weekday')?.value.toLowerCase()
      const hour = parseInt(timeParts.find((p: any) => p.type === 'hour')?.value || '0', 10)
      const minute = parseInt(timeParts.find((p: any) => p.type === 'minute')?.value || '0', 10)

      let isOutside = false
      if (weekday && (agent.business_hours.schedule as any)[weekday]) {
        const dayConfig = (agent.business_hours.schedule as any)[weekday]
        if (!dayConfig.active) {
          isOutside = true
        } else {
